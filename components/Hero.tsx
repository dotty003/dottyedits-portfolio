...
export const Hero: React.FC = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Main container tilts (Camera body rotation)
      containerRef.current.style.transform = `perspective(1000px) rotateY(${x * 12}deg) rotateX(${-y * 12}deg)`;

      // Parallax Effects - Elements move at different speeds to create depth
      ...
      if (sensorRef.current) {
        // Sensor moves slightly less than mid to appear "deep" inside
      ...
      if (lensGlassRef.current) {
         // Glass moves most, appearing closest to viewer
        lensGlassRef.current.style.transform = `translate(${x * 45}px, ${y * 45}px)`;
      }

      if (reflectionRef.current) {
        // Reflections move opposite to view direction (Simulating convex glass surface)
        // Scaling creates a "magnifying" feel at edges
      ...
      if (flareGroupRef.current) {
        // Flares move aggressively opposite to look direction to simulate light source angle
  ...
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden bg-black selection:bg-white selection:text-black">
      ...
      <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none">
        <div 
          ref={containerRef}
          className="relative w-[350px] h-[350px] md:w-[600px] md:h-[600px] transition-transform duration-100 ease-out will-change-transform opacity-60 md:opacity-100"
        >
            {/* 1. Lens Housing (Back Layer) */}
            <div 
              ref={lensBaseRef}
              className="absolute inset-0 rounded-full bg-[#050505] border border-neutral-800 shadow-2xl flex items-center justify-center transition-transform duration-100 ease-out"
            >
                {/* Decorative Rings */}
            ...
            {/* 2. Inner Mechanism (Middle Layer) */}
            <div 
              ref={lensMidRef}
              className="absolute inset-[15%] rounded-full bg-[#080808] border border-neutral-800 shadow-[0_0_50px_rgba(0,0,0,0.8)] flex items-center justify-center transition-transform duration-100 ease-out overflow-hidden"
            >
               {/* Detail lines mimicking lens barrel ridges */}
               ...
               {/* DIGITAL SENSOR: Iridescent Rectangle Deep Inside */}
               <div 
                 ref={sensorRef}
                 className="absolute w-[30%] h-[30%] bg-neutral-900 transition-transform duration-100 ease-out flex items-center justify-center overflow-hidden border border-neutral-800/50 rounded-sm"
               >
                  {/* Sensor Grid Pattern */}
            ...
            {/* 3. Front Glass Element (Front Layer) */}
            <div 
              ref={lensGlassRef}
              className="absolute inset-[28%] rounded-full bg-gradient-to-br from-neutral-800/20 to-black/80 overflow-hidden shadow-[inset_0_5px_20px_rgba(255,255,255,0.1)] border border-neutral-800 flex items-center justify-center transition-transform duration-100 ease-out backdrop-blur-[1px]"
            >
                ...
                {/* Dynamic Environmental Reflection (The Glint) */}
                <div 
                  ref={reflectionRef}
                  className="absolute -inset-full bg-gradient-to-tr from-transparent via-white/10 to-transparent blur-2xl rotate-45 pointer-events-none z-20 mix-blend-overlay transition-transform duration-100 ease-out"
                ></div>
                
                {/* Dynamic Lens Flares */}
                <div 
                  ref={flareGroupRef}
                  className="absolute inset-0 pointer-events-none transition-transform duration-100 ease-out z-30"
                >
                    {/* Primary Flare (Soft) */}
                ...
                {/* Fixed Specular Highlights (Simulating studio lighting reflections) */}
      ...
      <div className="relative z-10 container mx-auto px-6 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1 border border-white/20 rounded-full mb-8 backdrop-blur-md animate-fade-in-up bg-neutral-900/50 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
          ...
          <span className="text-xs uppercase tracking-widest text-white/80 font-bold">Open for Commissions</span>
        </div>
        ...
        <h1 className="text-6xl md:text-9xl font-black text-white mb-6 tracking-tighter leading-none animate-fade-in-up delay-100 mix-blend-difference drop-shadow-2xl">
          DOTTY<span className="text-neutral-500">.</span>EDITS
        </h1>
